<script lang="ts">
  ...
  import { loadConfig, saveConfig, type StoredConfig, type StoredDesign } from "./lib/configStorage.js";
  import {
    getKeycapFontOption,
    normalizeKeycapFontId,
  } from "./lib/keycapFonts";
  import { colorLabelForHex } from "$lib/colorPalette.js";
  import ColorPickerPopover from "$lib/components/ColorPickerPopover.svelte";
  import KeycapFontPicker from "$lib/components/KeycapFontPicker.svelte";
  import NumberOfKeysSelect from "$lib/components/NumberOfKeysSelect.svelte";
   ...
   */
  function getDefaultPositions(n: number) {
    const defaults = KEYCAP_POSITIONS[Math.min(7, Math.max(1, n)) - 1] ?? [];
    return defaults.map(([x, y, z]) => ({ x, y, z }));
  }

  /**
   * @param {string} name
   * @param {StoredConfig | null | undefined} [fromSaved] - optional config for migration (legacy flat shape or StoredConfig)
   * @returns {StoredDesign}
   */
  const DEFAULT_KEYCAP_SVG_ID = "heart.svg";

  function createDefaultDesign(
    name: string,
    fromSaved: StoredConfig | null | undefined = undefined,
  ) {
    const raw = fromSaved && typeof fromSaved === "object" ? fromSaved : {};
    const from = raw as Partial<StoredDesign>;
    const n = Math.min(7, Math.max(1, from?.numberOfKeys ?? 1));
    const hasExistingSvg =
      from?.keycapSvgByCount && Object.keys(from?.keycapSvgByCount ?? {}).length > 0;
    return {
      id:
        crypto.randomUUID?.() ??
      ...
      keycapFontId: normalizeKeycapFontId(from.keycapFontId),
    };
  }
  ...
  let svgPickerKeyIndex = $state<number | null>(null);
  ...
  onMount(() => {
    ...
    syncDownloadParam();
    window.addEventListener("popstate", syncDownloadParam);
    return () => window.removeEventListener("popstate", syncDownloadParam);
  });
  ...
  const textBorderColor = $derived(activeDesign?.textBorderColor ?? "#1f2937");
  ...
  const keycapFontId = $derived(
    normalizeKeycapFontId(activeDesign?.keycapFontId),
  );
  const keycapTypefaceUrl = $derived(
    getKeycapFontOption(keycapFontId).typefaceUrl,
  );
   ...
   * @param {number} index
   */
  function deleteDesign(index: number) {
  ...
  const snapshotBaseColorLabel = $derived(colorLabelForHex(objectColor));
  const snapshotKeycapColorLabel = $derived(colorLabelForHex(keycapColor));
  const snapshotLegendColorLabel = $derived(colorLabelForHex(textBorderColor));
  const snapshotKeycapFontLabel = $derived(
...
<div class="flex flex-col md:flex-row w-full h-full min-h-dvh overflow-hidden">
      ...
      >
        <div class="py-6 px-5 flex flex-col items-center text-center">
          ...
          <p class="m-0 mb-5 text-sm text-slate-500 leading-snug">
            Design your custom clicker keychains. Choose colors, add letters or
            icons to each key, and create multiple designs. Print. Personalize.
            Cherish.
          </p>
          <p
  ...
  >
    ...
    <div class="flex flex-col gap-2">
      <span class="text-[13px] font-medium text-slate-700">Number of keys</span>
      <NumberOfKeysSelect
        value={numberOfKeys}
        onChange={(n) => updateActiveDesign({ numberOfKeys: n })}
      />
    </div>
    <KeycapFontPicker
      {keycapFontId}
      onPick={(id: string) => updateActiveDesign({ keycapFontId: id })}
    />
    <div class="flex flex-col gap-2">
      <span class="text-[13px] font-medium text-slate-700">Base Color</span>
      <ColorPickerPopover
        currentColor={objectColor}
        onPick={(hex) => updateActiveDesign({ objectColor: hex })}
        title="Object color"
        ariaLabel="Choose object color"
      />
    </div>
    <div class="flex flex-col gap-2">
      <span class="text-[13px] font-medium text-slate-700">Keycaps color</span>
      <ColorPickerPopover
        currentColor={keycapColor}
        onPick={(hex) => updateActiveDesign({ keycapColor: hex })}
        title="Keycaps color"
        ariaLabel="Choose keycaps color"
      />
    </div>
    <div class="flex flex-col gap-2">
      <span class="text-[13px] font-medium text-slate-700">Legend color</span>
      <ColorPickerPopover
        currentColor={textBorderColor}
        onPick={(hex: string) => updateActiveDesign({ textBorderColor: hex })}
        title="Text and border color"
        ariaLabel="Choose text and border color"
        legendStyle={true}
      />
    </div>
    <label
    ...
    >
      <span class="text-[13px] font-medium text-slate-700">Show border</span>
      <input
        type="checkbox"
    ...
    <div class="flex flex-col gap-2">
      <span class="text-[13px] font-medium text-slate-700">Preset gallery</span>
      <p class="text-[11px] text-slate-500 m-0">
            ...
            onclick={() => {
              if (svgPickerKeyIndex !== null && activeDesign) {
                const next = keycapSvg.map(
                  (/** @type {string | null} */ s, /** @type {number} */ j) =>
          ...
          {#each SVG_ASSETS as asset}
            <button
              ...
              class="aspect-square p-0.5 border rounded md:rounded-lg bg-white cursor-pointer flex items-center justify-center transition-colors touch-manipulation {svgPickerKeyIndex !==
                null && keycapSvg[svgPickerKeyIndex] === asset.id
                ...
                if (svgPickerKeyIndex !== null && activeDesign) {
                  const next = keycapSvg.map(
                    (
                      /** @type {string | null} */ s,
                      /** @type {number} */ j,
                    ) => (j === svgPickerKeyIndex ? asset.id : s),
                  );
                  updateActiveDesign({
                    keycapSvgByCount: {
                      ...activeDesign.keycapSvgByCount,
            ...
            >
              <img src={asset.url} alt="" class="w-6 h-6 object-contain" />
            </button>
          {/each}
        </div>
      </div>
    ...
    >
      ...
      >
        ...
        <p class="m-0 mb-3 text-sm text-slate-600 leading-snug">
          Your snapshot has been saved. Check your <strong>Downloads</strong>
          folder (or <strong>Photos / Gallery</strong> on mobile) for the image.
        </p>
        <p class="m-0 mb-5 text-sm text-slate-600 leading-snug">
          When you order your fidget clicker from <strong>PixnPrints</strong>,
  ...
  {#if deleteConfirmIndex !== null}
    ...
    >
      ...
      >
        ...
        <p class="m-0 mb-5 text-sm text-slate-500 leading-snug">
          {designs[deleteConfirmIndex]?.name ?? "This design"} will be removed. This
          cannot be undone.
        </p>
  ...
  <div class="flex-1 min-w-0 flex flex-col overflow-hidden">
        ...
        {#each keycapLetters as letter, i}
          <div class="flex items-center gap-1.5">
            <label class="flex items-center gap-2 flex-1 min-w-0">
              <span class="text-xs font-medium text-slate-600 min-w-9"
                >Key {i + 1}</span
              >
              <input
                id="letter-input-{i}"
                ...
                oninput={(e) => {
                  ...
                  if (v && i + 1 < keycapLetters.length) {
                    requestAnimationFrame(() => {
                      document.getElementById(`letter-input-${i + 1}`)?.focus();
                    });
                  }
                }}
            ...
            <button
              ...
              onclick={() => (svgPickerKeyIndex = i as number)}
            >
              {#if keycapSvg[i]}
                <img
                  src={SVG_ASSETS.find((a) => a.id === keycapSvg[i])?.url}
                  alt=""
                  class="w-full h-full object-contain"
                />
              {:else}
                <span class="text-[10px] font-medium text-slate-400">SVG</span>
              {/if}
            </button>
          </div>
        {/each}
    ...
    >
      ...
      <ColorPickerPopover
        ...
        onPick={(hex) => updateActiveDesign({ keycapColor: hex })}
        title="Keycaps color"
        ariaLabel="Keycaps color"
      />
      <ColorPickerPopover
        compact
        legendStyle={true}
        currentColor={textBorderColor}
        onPick={(hex) => updateActiveDesign({ textBorderColor: hex })}
        title="Legend color"
    ...
    <div class="relative flex-1 min-h-0 min-w-0">
      <main class="canvas-wrap flex-1 min-h-0 min-w-0 h-full">
        <Canvas toneMapping={NoToneMapping}>
          <Scene
            ...
            {showBorder}
            {numberOfKeys}
            keycapPositions={keycapPositionsForScene as [number, number, number][]}
            {keycapLetters}
            keycapSvgUrls={keycapSvgUrlsForScene}
            {keycapSvgSizeMm}
            {keycapTypefaceUrl}
            snapshotReady={(fn) => {
              takeSnapshot = fn as () => void;
            }}
      ...
      <button
        ...
        onclick={() => designs.length > 1 && (deleteConfirmIndex = activeIndex)}
      >
        <svg
          class="w-5 h-5"
          fill="none"
